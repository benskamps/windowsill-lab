...
> ⚠ **Retracted and re-priced 2026-09-11 (audit item 2).** The paragraph that stood here from 2026-08-25 said the strongest uncatalogued crossing was **TIC 382028425 at SDE 8.048** and cited a null of **84,500 draws, max 8.049**. Both were products of a non-monotone estimator: an empty tail was priced at the rule-of-three bound `3/N` while a single exceedance was priced at the point estimate `1/N`, so one exceedance scored three times *better* than none, SDE 8.6 was promotable while SDE 50 was refused, and "strongest" meant *weakest crossing with exactly one exceedance*. SDE 10.14 and 12.31 sat in the same receipts. The estimator in `u_a01_attempt.empirical_fap` now grades every tail on the same one-sided 95 % Clopper–Pearson bound, which cannot rise with SDE. The re-analysis below is re-run on the **accepted** receipts only — five receipts whose own controls failed (three uniformity, two budget share; audit item 5) no longer contribute rows, because a permutation null that does not describe a sample's noise cannot price that sample's crossings.

The scramble campaign put the threshold region inside the sample — **325,000 null draws, max SDE 8.650, 13 draws at or above the SDE 8.0 threshold** — so every crossing this survey ever made can be given a *measured* false-alarm probability instead of an extrapolated one. The threshold sits **below** the null's maximum: a crossing between 8.0 and 8.65 is priced, never promotable. The decision rule was committed at 12:10Z on 2026-08-25 **before any real-target disposition was examined** (`4832d91`); the estimator correction is the only change to it since.

Result (2026-09-11): 11,990 searched rows over **11,814 distinct targets** produced **193 crossings at SDE ≥ 8** — of which **87 are catalogued planets**, which is the pipeline recovering real signal and is reported as calibration, not discovery. Of the 106 uncatalogued crossings the strongest is **TIC 219244444, SDE 12.31**, with zero exceedances in 325,000 draws — bound 9.2e-06, expected background **0.109** across the trials — just over the pre-registered ceiling of 0.1. It is independently dispositioned `harmonic-alias`, so it dies twice. Under this bound and this trials count no uncatalogued crossing can clear 0.1 until the null holds ~350,000 draws, and then only with an empty tail — which is the honest statement of what the survey can and cannot say about its own record.

**The claim that this survey already holds an undiscovered transit is killed.** Same verdict as 2026-08-25, reached by the right crossing this time. The shelf's empty is a measured empty with every exit counted.
