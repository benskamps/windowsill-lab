...
## 0 · Independent verification (2026-09-14) — read before sending

A fresh agent, forbidden to read this repository's code, wrote its own brute force from the OEIS definition alone (Duval factorisation from the textbook description, all 2^(n−1) compositions enumerated; one self-caught bug: "uniform" means equal *length*, per the entry's own COMMENTS, not equal words). Result: **all 25 published terms reproduced exactly**, and the conjecture a(n) = 2·A000041(n) − A000005(n) matches **all 15 proposed terms n = 26..40 exactly**. The live entry (revision history fetched 2026-09-14) has not changed since revision #15, 2021-06-20, still carries `keyword: more`, and holds 25 terms — the extension is still open. Direct (non-conjecture) brute force at **n = 26 over all 2²⁵ = 33,554,432 compositions: a(26) = 4868, exact** — the first new term confirmed by enumeration, not only by the conjecture (n = 27..30 were cut off by the box's memory watchdog). The agent's program is committed beside this file as `A329398-independent-bruteforce.py`.
