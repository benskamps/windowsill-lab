...
| 13 | **Found by `lab.weird`, blind — and worse than first logged.** The injection ladder is `DEPTHS = (0.002, 0.004, 0.010)`, and its output is censored at BOTH ends. Of 5,130 cells: **56.6% sit on the FLOOR** (0.002 — recovered the shallowest depth ever tried, so the true sensitivity is *better* than stored, left-censored), **7.3% on the CEILING** (0.010 — only the deepest recovered, right-censored), 26.0% correctly `None`, and **only 10.0% are interior measurements**. So **64% of the survey's sensitivity values are bounds stored as point values, censored in both directions** — the bias cannot even be signed. A three-rung ladder produces mostly censored output by construction. | HIGH | **OPEN** — mark censoring in the receipt; widen the ladder |
