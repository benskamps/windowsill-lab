...
| 13 | **Found by `lab.weird`, blind.** `d_min = 0.010` is the TOP RUNG of the injection ladder (`DEPTHS = (0.002, 0.004, 0.010)`), so a host reporting it recovered only the deepest transit anyone tried planting — a right-censored lower bound stored as a measurement. **377 of 5,130 ladder cells (7.3%) across 199 hosts (12.4%).** The prose is honest (`"sensitive to depth >= X%"`); the DATA is not, so every consumer computing on `d_min` reads a floor as a value. The survey already has the right concept — 1,336 cells correctly say `None` for no recovery — the boundary case is what leaks. | MEDIUM | **OPEN** — mark censored cells in the receipt |
