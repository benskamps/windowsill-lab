...
| 2 | `u_a01_attempt.empirical_fap` is non-monotone in SDE: the zero-exceedance bound `3/N` exceeds the one-exceedance estimate `1/N`, so SDE 8.60 is promotable and SDE 50 is refused. The kill verdict on the discovery question was arithmetically guaranteed before any data was seen, and it named SDE 8.048 "the strongest uncatalogued crossing" while SDE 10.14 sat in the same receipts. | SEVERE | **FIXED 2026-09-11** (branch `g01/fix-2-5-12`) — `empirical_fap` grades every tail on the one-sided 95 % Clopper–Pearson bound (`_binom_upper`, stdlib incomplete beta), monotone in SDE; `run()` reads `a04.SDE_THRESHOLD` and the accepted receipts only; verdict RETRACTED and re-priced in `UNKNOWNS.md` (strongest uncatalogued crossing is TIC 219244444 at SDE 12.31, background 0.109, still killed); receipt `reports/2026-09-11-u-a01.json`; 3 tests |
...
| 5 | Three receipts whose own uniformity control FAILED are published and counted. `check_a05` returns `None` for them ("every graded FAP is uninterpretable") but `_hunt_refusal`, the gate that actually runs at aggregation, accepts them. `hunt-2026-08-18-s2-1000.json` is one, and it is the **sole source** of lead TIC 77044472. Two more fail on budget over-share. | HIGH | **FIXED 2026-09-11** (branch `g01/fix-2-5-12`) — gates 10–12 of `check_a05` extracted to `checks.a05_control_verdict`; `_hunt_refusal` calls it for every schema≥1 receipt and refuses by name (`control:uniformity-failed` ×3, `control:budget-over-share` ×2 on the committed ledger — five receipts, 809 rows, one lead). Public block: `targets_searched` 13,350 → 12,541, leads 9 → 8. 4 tests, one pinned against the committed data |
| 6 | TIC 144122210 is documented in `a04.py:180-186` as an eclipsing binary ("missed being auto-disposed an EB by 0.13 sigma"; re-measured ~8.9 sigma in a duration-matched window) and is still `lead-awaiting-human-review`. Systematic: `secondary_window_phase` first appears 2026-08-29, so **7 of 9 lead stars** had their secondary measured in a fixed window `a04.py` calls biased toward zero, landing hardest on short transits. | HIGH | **CLOSED 2026-09-12** — TIC 144122210 searched in sector 31 (`followup-144122210-s31.json`, accepted): the machine dispositioned it `eclipsing-binary-secondary`; the shelf now records the gate fired against the star. A human ruling (refuted) is drafted for Ben. The remaining pre-08-29 leads are otherwise refuted or parked with the gap named |
...
| 12 | **Ours, not the audit's.** `docs/preregistrations/2026-09-10-u-a01-sector-96.md` justifies its SDE >= 8.0 threshold with "84,500 draws, maximum 8.049" — taken from `UNKNOWNS.md`. The live null is **325,000 draws with a sector-3 maximum of 8.6499**. The running sector-96 hunt inherits a threshold below the measured null maximum. | HIGH | **FIXED 2026-09-11** (branch `g01/fix-2-5-12`) — every receipt now carries `pooled_null` (draws, max, exceedances at threshold, bound at threshold), measured by the driver at receipt time, never quoted. The v1 sector-96 run is SET ASIDE, not resumed; `docs/preregistrations/2026-09-11-u-a01-sector-96-v2.md` supersedes it with the real null (325,000 draws, max 8.6499) and states in advance that a crossing in 8.0–8.65 is priced, never promotable |
