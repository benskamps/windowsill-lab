...
def _plot_calibration(report: dict) -> str:
    ...
    elif exp.startswith("C05"):
        fig, ax = plt.subplots(figsize=(8, 4))
        wins = report.get("windows") or []
        pos = [w["position"] + 1 for w in wins]      # +1: log axis, position 0
        cost = [max(w.get("wall_seconds", 0.0), 1e-4) for w in wins]
        ok = [w.get("match") for w in wins]
        ax.plot([p for p, o in zip(pos, ok) if o],
                [c for c, o in zip(cost, ok) if o],
                "o", color="#6f7952", ms=4, label="window matches Machin")
        bad = [(p, c) for p, c, o in zip(pos, cost, ok) if not o]
        if bad:
            ax.plot(*zip(*bad), "x", color="#a04b3c", ms=7, label="MISMATCH")
        deep = report.get("deep") or {}
        if deep.get("position"):
            ax.plot([deep["position"] + 1], [max(deep.get("wall_seconds", 0), 1e-4)],
                    "D", color="#b88963", ms=7,
                    label=f"deep window @ {deep['position']:,}")
        ax.set_xscale("log"); ax.set_yscale("log")
        ax.set_xlabel("hex-digit position (1-shifted)")
        ax.set_ylabel("extraction wall seconds")
        ax.set_title("the cost of teleporting into π")
        ax.legend(frameon=False, fontsize=8)
    elif exp.startswith("A03"):
        # Gravitational-wave event: detector SNR comparison (real vs control injection)
        fig, ax = plt.subplots(figsize=(8, 4))
        dets = report.get("detectors") or []
        if dets:
            detector_names = [d["detector"] for d in dets]
            real_snr = [d["real"]["peak_snr"] for d in dets]
            control_snr = [d["control"]["peak_snr"] for d in dets]
            x = np.arange(len(detector_names))
            ax.plot(x, real_snr, "o-", ms=6, lw=1.5, color="#7a9b56",
                    label="real event")
            ax.plot(x, control_snr, "s-", ms=6, lw=1.5, color="#667c86",
                    label="control injection")
            ax.set_xticks(x)
            ax.set_xticklabels(detector_names)
            ax.set_ylabel("peak SNR")
            ax.set_title(f"detector network SNR — {report.get('event')}")
            ax.legend(frameon=False, fontsize=8)
        else:
            ax.axis("off")
            ax.text(.5, .5, "No detector data available", ha="center", va="center",
                    fontsize=14, color="#6f5b48")
    elif exp.startswith("A04"):
        # Blind transit search: SDE distribution (searched targets vs false alarms)
        fig, ax = plt.subplots(figsize=(8, 4))
        searched = report.get("searched") or []
        false_alarms = report.get("false_alarm_sde") or []
        if searched or false_alarms:
            searched_sde = [s.get("sde") for s in searched if s.get("sde") is not None]
            threshold = float(report.get("sde_threshold", 8.0))
            if searched_sde:
                ax.hist(searched_sde, bins=15, alpha=0.6, color="#7a5c3e",
                        label=f"targets ({len(searched_sde)})")
            if false_alarms:
                ax.hist(false_alarms, bins=15, alpha=0.6, color="#8d8175",
                        label=f"false alarm floor ({len(false_alarms)})")
            ax.axvline(threshold, ls="--", lw=1.2, color="#b88963",
                       label=f"threshold {threshold}")
            ax.set_xlabel("signal detection efficiency (SDE)")
            ax.set_ylabel("count")
            ax.set_title("blind transit search — SDE distribution")
            ax.legend(frameon=False, fontsize=8)
        else:
            ax.axis("off")
            ax.text(.5, .5, "No search data available", ha="center", va="center",
                    fontsize=14, color="#6f5b48")
    elif exp.startswith("K03"):
        # Field theory critical exponents: Daido vs Hong branches
        fig, ax = plt.subplots(figsize=(8, 4))
        below = report.get("columns_below") or []
        above = report.get("columns_above") or []
        fit_below = report.get("fit_below") or {}
        fit_above = report.get("fit_above") or {}
        eps = np.asarray(report.get("eps_grid", []), dtype=float)
        if eps.size > 0 and (below or above):
            # Plot the measured gamma for both branches
            below_chi = [b.get("chi") for b in below if b.get("chi") is not None]
            above_chi = [a.get("chi") for a in above if a.get("chi") is not None]
            if len(below_chi) > 0:
                ax.plot(eps[:len(below_chi)], below_chi, "o-", ms=4, lw=1.0,
                        color="#7d8f68", label="subcritical (ε < K_c)")
            if len(above_chi) > 0:
                ax.plot(eps[:len(above_chi)], above_chi, "s-", ms=4, lw=1.0,
                        color="#667c86", label="supercritical (ε > K_c)")
            ax.set_xscale("log"); ax.set_yscale("log")
            ax.set_xlabel("driving magnitude  ε")
            ax.set_ylabel("linear-response susceptibility  χ")
            ax.set_title("field theory exponents — Daido vs Hong")
            ax.legend(frameon=False, fontsize=8)
        else:
            ax.axis("off")
            ax.text(.5, .5, "No column data available", ha="center", va="center",
                    fontsize=14, color="#6f5b48")
    elif exp.startswith("M18"):
        # Directed percolation: critical exponent bracket via density-decay
        fig, ax = plt.subplots(figsize=(8, 4))
        windows = report.get("windows") or []
        if windows:
            # Plot the leading (graded) window which is the measurement
            graded = next((w for w in windows if w.get("graded")), None)
            if graded:
                p_range = [report.get("p_low"), report.get("p_high")]
                delta_at_low = graded.get("delta_at_p_low")
                delta_at_high = graded.get("delta_at_p_high")
                bracket = report.get("bracket") or [delta_at_high, delta_at_low]
                ax.plot(p_range, [delta_at_low, delta_at_high], "o-", ms=8, lw=2.0,
                        color="#7a5c3e", label="measured density decay")
                ax.fill_between([p_range[0] - 0.0001, p_range[1] + 0.0001],
                               bracket[0], bracket[1], alpha=0.2, color="#b88963")
                ax.axhline(report.get("benchmark_delta", 0.4505), ls="--", lw=1.2,
                          color="#7a9b56", label="DP benchmark 0.4505")
                ax.axhline(report.get("mean_field_delta", 1.0), ls=":", lw=1.0,
                          color="#8d8175", label="mean-field 1.0")
                ax.set_xlabel("critical parameter  p")
                ax.set_ylabel("density-decay exponent  δ")
                ax.set_title("directed percolation bracket")
                ax.legend(frameon=False, fontsize=8)
            else:
                ax.axis("off")
                ax.text(.5, .5, "No graded window available", ha="center", va="center",
                        fontsize=14, color="#6f5b48")
        else:
            ax.axis("off")
...
def render_calibration(report: dict, date: str | None = None) -> Path:
    """Render M16/C01/A01/A03/A04/K03/M18/I01 reports through one quiet four-plant template."""
    from .publish import today_local
    date = date or today_local()
    _ensure_home()
